...
const posDevicesActionMenu = (props) => {
    const [anchorEl, setAnchorEl] = useState(null);
    const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
    const posdeviceData = props?.deviceData;
    ...
    const handleEdit = () => {
        handleClose();
        navigate("/edit-pos", { state: { deviceData: toJS(posdeviceData) } });
    };

    const handleView = () => {
        handleClose();
        navigate("/pos-details", { state: { deviceData: toJS(posdeviceData) } });
    };
    ...
    const handleDeleteConfirm = async () => {
        try {
            await posdevicesStore.deletePosDevices(posdeviceData.id);
            // Refresh the devices list after successful deletion
            await posdevicesStore.fetchPosDevices();
            setDeleteDialogOpen(false);
        } catch (error) {
            // Error is handled in the store, we just need to close the dialog
            // The error will be available in posdevicesStore.error
    ...
    return (
        <>
            ...
            <ConfirmationDialog
                ...
                message={`Are you sure you want to delete "${posdeviceData?.device_name}"? This action cannot be undone.`}
                confirmText="Delete"
                cancelText="Cancel"
                confirmColor="error"
                icon="delete_outline"
...
export default posDevicesActionMenu;