...
const AgentActionMenu = ({ agent }) => {
    ...
    const handleEdit = () => {
        handleClose();
        // Navigate to edit route with the agent id
        navigate("/edit-agent", { state: { agentData: agent } });

    };

    const handleView = () => {
        handleClose();
        navigate("/agent-details", { state: { agentData: agent } });
    };
