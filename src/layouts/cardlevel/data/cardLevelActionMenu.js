...
const cardLevelActionMenu = (props) => {
    const [anchorEl, setAnchorEl] = useState(null);
    const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
    const cardLevelData = props?.cardLevelData;
    ...
    const handleEdit = () => {
        handleClose();
        navigate("/edit-cardlevel", { state: { cardLevelData: toJS(cardLevelData) } });
    };
    ...
    const handleDeleteConfirm = async () => {
        try {
            await cardStore.deleteCardLevel(cardLevelData.id);
            // Refresh the card level list after successful deletion
            await cardStore.fetchCardLevel();
    ...
    return (
        <>
            ...
            <ConfirmationDialog
                ...
                title="Delete Card Level"
                message={`Are you sure you want to delete "${cardLevelData?.name}"? This action cannot be undone.`}
                confirmText="Delete"
                cancelText="Cancel"
                confirmColor="error"
                icon="delete_outline"
...
export default cardLevelActionMenu;