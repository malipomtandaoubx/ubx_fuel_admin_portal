...
const cardTypeActionMenu = (props) => {
    const [anchorEl, setAnchorEl] = useState(null);
    const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
    const cardTypeData = props?.cardTypeData;
    ...
    const handleEdit = () => {
        handleClose();
        navigate("/edit-cardtype", { state: { cardTypeData: toJS(cardTypeData) } });
    };
    ...
    const handleDeleteConfirm = async () => {
        try {
            await cardStore.deleteCardType(cardTypeData.id);
            // Refresh the devices list after successful deletion
            await cardStore.fetchCardType();
    ...
    return (
        <>
            ...
            <ConfirmationDialog
                ...
                message={`Are you sure you want to delete "${cardTypeData?.name}"? This action cannot be undone.`}
                confirmText="Delete"
                cancelText="Cancel"
                confirmColor="error"
                icon="delete_outline"
...
export default cardTypeActionMenu;